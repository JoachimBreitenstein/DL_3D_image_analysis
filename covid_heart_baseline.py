...
for epoch in range(NUM_EPOCHS):
    ...
    dice_sum += compute_dice(one_hot(pred.argmax(dim=1, keepdim=True), num_classes=2, dim=1), one_hot(label.unsqueeze(1), num_classes=2, dim=1), include_background=False).mean(dim=1).sum()
        
    mean_val_loss += loss * len(image_b)
    num_samples += len(image_b)
    val_dice = dice_sum.item() / num_samples
    all_val_dices.append(val_dice)
    step += 1
    ...
    if mean_val_loss.item() < best_val_loss:
        ...
        }, 'Models/model_checkpoint_3D.pth')

with open('3D_baseline.txt', 'w') as f:
...
plt.legend()
plt.savefig('Plots/losses_3D_100.png')