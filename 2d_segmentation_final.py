...
def grab_2d_slice(batch):
    new_batch = batch.copy()
    new_batch['image'] = new_batch['image'][:,:,:,:,new_batch['image'].shape[3] // 2]
    new_batch['label'] = new_batch['label'][:,:,:,:,new_batch['label'].shape[3] // 2]
    new_batch['mask'] = new_batch['mask'][:,:,:,:,new_batch['mask'].shape[3] // 2]

    for k, mask in enumerate(batch['mask']):
        x = torch.sum(mask[:,:,:,mask.shape[2] // 2])
        y = torch.sum(mask[:,:,mask.shape[2] // 2,:])
        z = torch.sum(mask[:,mask.shape[2] // 2,:,:])

        if torch.max(torch.tensor([x,y,z]))==x:
            new_batch['image'][k] = batch['image'][k,:,:,:,batch['image'].shape[3] // 2]
            new_batch['label'][k] = batch['label'][k,:,:,:,batch['label'].shape[3] // 2]
            new_batch['mask'][k] = batch['mask'][k,:,:,:,batch['mask'].shape[3] // 2]

        if torch.max(torch.tensor([x,y,z]))==y:
            new_batch['image'][k] = batch['image'][k,:,:,batch['image'].shape[3] // 2,:]
            new_batch['label'][k] = batch['label'][k,:,:,batch['label'].shape[3] // 2,:]
            new_batch['mask'][k] = batch['mask'][k,:,:,batch['mask'].shape[3] // 2,:]

        if torch.max(torch.tensor([x,y,z]))==z:
            new_batch['image'][k] = batch['image'][k,:,batch['image'].shape[3] // 2,:,:]
            new_batch['label'][k] = batch['label'][k,:,batch['label'].shape[3] // 2,:,:]
            new_batch['mask'][k] = batch['mask'][k,:,batch['mask'].shape[3] // 2,:,:]
...
for epoch in range(NUM_EPOCHS):
    ...
    dice = 1-loss
    dice_sum += dice 
    all_dice_metrics.append(dice_sum)
    
    val_time = time() - t0
    eval_times.append(val_time)
    ...
    if mean_val_loss.item() < best_val_loss:
        ...
        }, '2D_segmentation/model_2d_checkpoint.pth')

with open('2D_segmentation/2D_baseline.txt', 'w') as f:
    ...
    f.write('all val dices: ' + str(all_val_dices) + '\n')
